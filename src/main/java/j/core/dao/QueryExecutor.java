...
public class QueryExecutor implements Runnable{
	...
	protected void execute(String uuid,String sql,boolean syn){
		...
		if(!syn) return;
		while(tasks.containsKey(uuid)) Global.sleep10Millis();
	}
	...
	private void ensureDAO(){
		try{
			if(dao==null||dao.isClosed()){
				dao=DAOs.create(this.databaseName, this.getClass(), true, -1);
			}
		}catch(Exception e){
	...
	public void run() {
		while(running){
			...
			List<Query> temp=tasks.listValues();
			...
			while(!temp.isEmpty()){
				...
				try{
					if(query.type==Query.TYPE_SQL_NO_RESULT){
						...
						results.put(query.uuid, Integer.valueOf(count));
