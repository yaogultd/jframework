...
public class Server implements Runnable{
	...
	private ConcurrentList<Client> clients=new ConcurrentList<Client>();
	 ...
	 * @return
	 */
	public ConcurrentList<Client> getClients() {
	 ...
	 * @return
	 */
	public Client getClientLatestComm(String id) {
		Client client=null;
		for(int i=0;i<clients.size();i++) {
			Client c=(Client)clients.get(i);
	 ...
	 * @return
	 */
	public Client getClientOfUuid(String uuid) {
		Client client=null;
		for(int i=0;i<clients.size();i++) {
	...
	public void closeClientBefore(String id,String beforeUuid) {
		Client client=getClientOfUuid(beforeUuid);
		for(int i=0;i<clients.size();i++) {
			Client c=clients.get(i);
			if(c.getId().equals(id)
	...
	private int clientOnIP(String ip) {
		int count=0;
		ConcurrentList<Client> clients=getClients();
		for(int i=0;i<clients.size();i++) {
			Client c=clients.get(i);
			...
			while(!Startup.isDestroyed()) {
				try {
					...
					if(clientOnIP(socket.getInetAddress().getHostAddress())>=this.maxClientsPerIp) {
						...
						Client client=(Client)this.getClient().getConstructor(new Class[] {Socket.class,long.class,long.class,Object[].class}).newInstance(new Object[] {socket,this.getClientMaxIdle(),this.mustSendAfterConnectedWithin,this.args});
