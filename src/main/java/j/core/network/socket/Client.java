 ...
 * 2019年3月30日
 *
 * <b>功能描述</b> 当启动一个Server在指定端口监听客户端连接时，必须指定一个类来处理特定业务相关的客户端交互，该类必须是Client的子类
 */
public class Client implements Runnable{
	 ...
	 */
	public Client(Socket socket, long maxIdle) {
		this.uuid=JUtilUUID.genUUID();
		this.socket=socket;
	 ...
	 */
	public Client(Socket socket, long maxIdle, long mustSendAfterConnectedWithin) {
		this.uuid=JUtilUUID.genUUID();
		this.socket=socket;
		this.addr=socket.getInetAddress();
	 ...
	 * @param args 自定义业务参数
	 */
	public Client(Socket socket, long maxIdle, long mustSendAfterConnectedWithin, Object[] args) {
		this.uuid=JUtilUUID.genUUID();
		this.socket=socket;
		this.addr=socket.getInetAddress();
		this.mustSendAfterConnectedWithin=mustSendAfterConnectedWithin;
	...
	public void setLastActive(){
		this.lastActive=SysUtil.getNow();
		if(this.interactions==Long.MAX_VALUE) this.interactions=0;
		this.interactions++;
	}
