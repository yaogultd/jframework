...
public abstract class JCacheUnit implements Serializable{
	...
	protected JCacheInitializer initializer;
	protected int lifeCircle;//缓存单元生命周期类型（见JCache）
	protected long timeout;//临时缓存超时时间
	 ...
	 * @param initializer
	 */
	public JCacheUnit(int lifeCircle, long timeout, JCacheInitializer initializer){
	 ...
	 */
	public void init(){
		if(this.initializer != null) this.initializer.initialize(this);
	}
	...
	public boolean isTimeout() throws Exception{
		if(this.lifeCircle==JCache.LIFECIRCLE_TEMPORARY
				&&this.timeout>0
				&&SysUtil.getNow()-this.updateTime>timeout){
			return true;
		}
		return false;
	}
