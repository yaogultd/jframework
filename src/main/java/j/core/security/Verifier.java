...
public class Verifier{
	 ...
	 * @throws Exception
	 */
	public static String allotUuid(String uuid, String ip) throws Exception{
		if(uuid==null) uuid=JUtilUUID.genUUID();

		VerifyCodeBean vcb=VerifyCode.get(uuid,
				ip,
				ip,
				...
				200,
				VerifierSetting.getMaxTries());

		log.log("allotUuid => "+uuid+" => "+ JUtilBean.bean2Json(vcb), -1);
	...
	public static boolean allotted(String uuid) throws Exception{
		...
		return VerifyCode.exists(uuid)!=null;
	}
	 ...
	 * @throws Exception
	 */
	public static boolean isCorrect(String uuid, String ip, String code) throws Exception{
		if(uuid==null||code==null) return false;
		return VerifyCode.check(uuid, ip, code,true);
	}
	...
	public static void writeImage(String uuid,HttpServletResponse response) {
		try{		
			VerifyCodeBean vcb=VerifyCode.exists(uuid);
