...
public class SysUtil {
	...
	public static boolean forceHttps(){
		return "true".equals(AppConfig.getPara("SYSTEM", "REDIRECT_TO_HTTPS"));
	}
	...
	public static void redirectByFormSubmit(HttpServletRequest httpRequest,
		...
		redirect += "</head>\r\n";
		redirect += "<body>\r\n";
		redirect += "<form id=\"redirect\" name=\"redirect\" action=\"" + action + "\" method=\"" + method + "\">\r\n";
	...
	public static void redirect(HttpServletRequest httpRequest, HttpServletResponse httpResponse, String url) throws IOException, ServletException {
		...
		if (url.startsWith("http://")
			...
			httpResponse.sendRedirect(redirectHttps(url));
		} else {//如果是系统内部地址
			//强制跳转https
			if(forceHttps()){
				url = JUtilString.appendUrl(JUtilString.getUrlBase(httpRequest.getRequestURL().toString()), url);
				if(!url.startsWith("https")) url="https"+url.substring(4);
			}
	 ...
	 * @return
	 */
	public static String redirectHttps(String url){
		if(forceHttps()
				&& url.startsWith("http")
				&& !url.startsWith("https")){
	...
	public static void forward(HttpServletRequest httpRequest,HttpServletResponse httpResponse, String url)throws ServletException, IOException {
		try{
			//log.log("try to forwrod url => "+url+" => "+httpResponse.encodeRedirectURL(url), -1);
			httpRequest.getRequestDispatcher(httpResponse.encodeRedirectURL(url)).forward(httpRequest,httpResponse);
		}catch (Exception e){
			log.log("failed to forwrod url => "+url+" => "+httpResponse.encodeRedirectURL(url), -1);
	...
	public static void forwardI18N(HttpServletRequest httpRequest,HttpServletResponse httpResponse, String url)throws ServletException, IOException {
		try{
			//log.log("try to forwardI18N url => "+url+" => "+httpResponse.encodeRedirectURL(url), -1);
			...
			if(start>0&&end>start){//页面中指定了多语言分组
				...
				content=I18N.convert(content,group,httpRequest);
			}else{
				content=I18N.convert(content,httpRequest);
			}
			...
			log.log("failed to forwardI18N url => "+url+" => "+httpResponse.encodeRedirectURL(url), -1);
			log.log(e, Logger.LEVEL_ERROR);
		}
	}
	 ...
	 * @return
	 */
	public static void setRequestUriOriginal(HttpServletRequest request) {
	 ...
	 * @return
	 */
	public static String getRequestUriOriginal(HttpServletRequest request) {
	...
	public static String getRequestURLBase(HttpServletRequest request){
		return getRequestURLBase(request.getRequestURL().toString(), forceHttps());
	}
	...
	public static String getRequestURLBase(String url){
		return getRequestURLBase(url, false);
	}
	 ...
	 * @return
	 */
	public static String getRequestURLBase(String url, boolean forceHttps){
		int i=url.indexOf("://");
		if(i<0) return null;
		
		String p=url.substring(0,i);
		if(forceHttps) p="https";
	...
	public static String getRequestURLWithoutParams(HttpServletRequest request){
		return JUtilString.appendUrl(getRequestURLBase(request), getRequestUriOriginal(request));
	}
	...
	public static String getRequestURL(HttpServletRequest request,String excludePrefix){
		String url=getRequestURLWithoutParams(request);
	 ...
	 *
	 * @param request
	 * @param excludePrefixs 排除以excludePrefix开头的参数
	 * @return
	 */
	...
	public static String getHttpParameter(HttpServletRequest request,String name){
		...
		if("application/x-www-form-urlencoded".equalsIgnoreCase(request.getContentType())
				&& JUtilString.isBlank(SysUtil.getHttpHeader(request, "Request_by_ajax"))){
			try{
	...
	/**
	 * 当前时间与UTC时间 1970年1月1日 00:00:00之间的差值，以毫秒为单位。
	 * @return
	 */
	 ...
	 * @throws Exception
	 */
	public static void outHttpResponse(HttpServletResponse response,String s){
		try {
	 ...
	 * @throws Exception
	 */
	public static void outHttpResponse(HttpServletResponse response,int s){
		try {
			response.setContentType("text/html;charset="+SysConfig.sysEncoding);
			response.setContentLength(-1);
			PrintWriter out=response.getWriter();
	 ...
	 * @throws Exception
	 */
	public static void outHttpResponse(HttpServletResponse response,String s,int contentLength){
		try {
			response.setContentType("text/html;charset="+SysConfig.sysEncoding);
	 ...
	 * @throws Exception
	 */
	public static void outHttpResponse(HttpServletResponse response,int s,int contentLength){
		try {
			response.setContentType("text/html;charset="+SysConfig.sysEncoding);
			response.setContentLength(contentLength);
			PrintWriter out=response.getWriter();
