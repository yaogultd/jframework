...
public class Upload {
	...
	private List<UploadedFile> uploadedFiles;//解析出的文件
	...
	private Map<String, String> otherParameters;//key - value pair。从上传内容中解析出的非文件字符串参数，即网页中和file一起使用的input、textarea等输入元素，不包括form的action中带的参数，form的action中带的参数直接用request.getParameter获得
	...
	public Upload(HttpServletRequest _request,String _path, String _encoding,int _allowFileSize,boolean saveInTempFile) throws IOException {
		...
		//log.log(JUtilInputStream.string(sis),-1);
	}		
...
--AEAoI1YFLSqY00gzaSXTeUySEzUQAnwiaWb--
				...
				if (line.indexOf(mimeBoundary)<0) {
					...
					if(line.toLowerCase().indexOf("filename")==-1
						...
						if(!this.saveInTempFile){
							...
							this.fileName_Saved = calculateFileName_Saved();
							file = new File(JUtilString.appendPath(destinationPath, fileName_Saved));
							file.createNewFile();					
						}else{
	...
	private void getFileExt() {
		int i = fileName_Uploading.lastIndexOf(".");
		if (i > -1) {
			fileExt_Uploading = fileName_Uploading.substring(i + 1).toLowerCase();
	...
	private String calculateFileName_Saved() {
		...
		return JUtilUUID.genUUID()+"."+fileExt_Uploading;
	}
	...
	public UploadedFile[] getUploadedFiles(String paraName){
		...
		for(int i=0;i<uploadedFiles.size();i++){
			UploadedFile f=uploadedFiles.get(i);
